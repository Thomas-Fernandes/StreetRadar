...
const CoverageChart: React.FC<CoverageChartProps> = ({
    ...
    const processDataForChart = useCallback(
        (data: CoverageData[], filters?: ChartFilters) => {
            ...
            filteredData.forEach((item) => {
                ...
                const value =
                    filters?.metric === 'panoramas' ? item.panorama_count : item.km_traces;
            ...
            const datasets = topCountries.map((country: string, index: number) => {
                ...
                return {
                    ...
                    data: countryData,
                    backgroundColor:
                        colors.countries[index] || colors.countries[colors.countries.length - 1],
                    borderColor:
                        colors.countries[index]?.replace('0.8', '1') ||
                        colors.countries[colors.countries.length - 1].replace('0.8', '1'),
                    borderWidth: 2,
    ...
    const options = {
        ...
        plugins: {
            title: {
                ...
                text:
                    title ||
                    `Street View ${filters?.metric === 'panoramas' ? 'Panoramas' : 'Coverage'} Evolution`,
                font: {
        ...
        >
            ...
            {showLegend && chartData?.allCountriesData && (
              ...
              `}
                        ...
                        >
                            {chartData.allCountriesData.map(
                                (country: { name: string; total: number }, index: number) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{
                                                ...
                                                padding: '4px 6px',
                                                borderRadius: '4px',
                                                backgroundColor: isTopCountry
                                                    ? 'rgba(155, 68, 52, 0.05)'
                                                    : 'transparent',
                                                border: isTopCountry
                                                    ? '1px solid rgba(155, 68, 52, 0.2)'
