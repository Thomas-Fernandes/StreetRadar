...
export class StreetViewDetectionCanvas {
    ...
    static async detectStreetViewAt(
        ...
        const activeConfigs = this.providerConfigs.filter((config) =>
            activeProviders.includes(config.name)
        );
    ...
    private static checkAppleMVTLayer(map: L.Map): boolean {
        ...
        map.eachLayer((layer: L.Layer) => {
            ...
            if (
                layerWithOptions.options?.attribution?.includes('Apple Look Around') ||
