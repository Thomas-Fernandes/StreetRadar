...
export class ApplePMTilesService {
    ...
    private static async fetchTileJSON(): Promise<TileJSONMetadata> {
        try {
            ...
            throw new Error(
                `Unable to load Apple PMtiles metadata: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }
    ...
    static getMVTTileUrl(x: number, y: number, z: number): string {
        return ApplePMTilesService.MVT_URL_TEMPLATE.replace('{x}', x.toString())
